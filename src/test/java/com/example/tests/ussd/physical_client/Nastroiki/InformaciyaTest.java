...
import static com.example.api.ussd.constants.TextMessageConfig.NASTROIKI_INFORMACIYA_RU;
import static com.example.api.ussd.constants.TextMessageConfig.NASTROIKI_INFORMACIYA_UZ;
import static com.example.api.ussd.constants.UserConfig.BASE_CLIENT;

public class InformaciyaTest extends UssdBaseTest {
    @Autowired
    ...
    public Stream<Arguments> cases() {
        return Stream.of(
                Arguments.of(BASE_CLIENT, "RU", NASTROIKI_INFORMACIYA_RU),
                Arguments.of(BASE_CLIENT, "UZ", NASTROIKI_INFORMACIYA_UZ)
        );
    }

    
    @DisplayName("Раздел 'Настройки -> Информация'")
    @ParameterizedTest
    @MethodSource("cases")
    void changeLangTest(String phoneNum, String lang, String expectedMenu){
        ...
        step("Переход в раздел 'Информация'", () -> {
            // указываем раздел 6 - Информация
            requestResponse.setBody("6");
